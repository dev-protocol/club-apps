<template>
  ...
  >
    <section class="flex flex-col">
      ...
      <div class="mb-8">
        ...
        <button @click="approve" v-if="approveNeeded" class="rounded-sm border bg-gray-600 p-2 px-4">
          Sign with wallet and approve
        </button>
        <button v-if="!approveNeeded" disabled class="rounded-sm border border-gray-400 bg-gray-600 p-2 px-4 text-gray-400">
          You've already approved
        </button>
      </div>
      <div class="mb-8">
        <h3 class="mb-4 text-2xl">Stake</h3>
        <button @click="submitStake" class="rounded-sm border bg-gray-600 p-2 px-4">
  ...
  <section style="height: calc(100vh - 74px)" class="flex flex-col items-center justify-center" v-if="stakeSuccessful">
...
<script lang="ts">
import { clientsDev, positionsCreate, positionsCreateWithEth } from '@devprotocol/dev-kit/agent'
...
export default defineComponent({
    ...
    mounted() {
        ...
        if (this.currency === CurrencyOption.ETH) {
          ...
          if (connection) {
            const sub = zip(connection.provider, connection.account).subscribe(async ([provider, account]) => {
              await whenDefinedAll([provider, account, this.destination], async ([prov, userAddress, destination]) => {
    ...
    methods: {
        async approve() {
          ...
          if (connection) {
            const sub = zip(connection.provider).subscribe(async ([provider]) => {
              await whenDefinedAll([provider, this.destination], async ([prov, destination]) => {
                const [l1, _] = await clientsDev(prov)
                const res = await l1?.approve(destination, constants.MaxUint256.toString());
                if (res) {
                  await res?.wait();
        ...
        async checkApproved(provider: providers.Provider, userAddress: string, destination: string) {
            const [l1, _] = await clientsDev(provider)
            const allowance = BigNumber.from((await l1?.allowance(userAddress, destination)) ?? 0)
          ...
          if (connection) {
            const sub = zip(connection.provider, connection.account).subscribe(async ([provider, account]) => {
              await whenDefinedAll([provider, account, this.destination], async ([prov, userAddress, destination]) => {
                ...
                if (this.currency === CurrencyOption.ETH) { // handle ETH stake
                  const res = await whenDefinedAll([provider, account, this.amount, this.destination], ([prov, from, amount, destination]) => positionsCreateWithEth({
                      provider: prov,

                      destination,
                      ethAmount: utils.parseUnits(amount.toString(), 18).toString(),
                  }));
                ...
                } else { // handle DEV stake
                  const res = await whenDefinedAll([provider, account, this.amount, this.destination], ([prov, from, amount, destination]) => positionsCreate({
                      provider: prov,
                      from,
                      destination,
